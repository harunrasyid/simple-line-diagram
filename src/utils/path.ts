...
export const generateOctilinearPaths = (
  ...
  if (layout) {
    layout.connections.forEach((conn) => {
      const existing = tripLaneMap.get(conn.tripId) || { inbound: 0, outbound: 0 };
  ...
  const tripConnections = new Map<string, { inbound: Connection[]; outbound: Connection[] }>();
  if (layout) {
    layout.connections.forEach((conn) => {
      const existing = tripConnections.get(conn.tripId) || { inbound: [], outbound: [] };
      if (conn.direction === "inbound") {
  ...
  const generatePath = (
    ...
    const dirConnections = direction === "inbound" ? connections?.inbound : connections?.outbound;
    const baseY = direction === "inbound" ? inboundY : outboundY;

    console.log(`[Path] Generating ${direction} path for trip ${tripId}:`, validStops);
    ...
    for (let i = 0; i < validStops.length; i++) {
      ...
      if (i < validStops.length - 1) {
        ...
        const conn = dirConnections?.find(c => c.from === currentId && c.to === nextId);
        
        console.log(`[Path] Connection ${currentId}->${nextId}:`, conn ? { isExpress: conn.isExpress, lane: conn.lane } : 'not found');
        
        // Check if this is an express connection that needs to route around stops
        if (conn && conn.isExpress && tripLane) {
          const tripLaneY = direction === "inbound" 
            ? baseY - conn.lane * laneHeight
            : baseY + conn.lane * laneHeight;
          
          console.log(`[Path] Express connection: tripLaneY=${tripLaneY}, current.y=${current.y}, diff=${Math.abs(tripLaneY - current.y)}`);
          ...
          if (Math.abs(tripLaneY - current.y) > 1) {
            ...
            console.log(`[Path] Detour points: (${detourX1}, ${tripLaneY}), (${detourX2}, ${tripLaneY})`);
